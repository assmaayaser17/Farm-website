    ...
    <nav id="navbar" class="fixed top-0 left-0 w-full z-50 px-6 py-4 border-b bg-transparent transition-all duration-500">
        <div class="max-w-7xl mx-auto flex justify-between items-center relative">
            ...
            <ul class="hidden md:flex gap-6 items-center">
                ...
                <li><a href="{{ route('categories.index') }}" class="relative group text-xl font-medium text-green-600 transition-colors">{{ __('messages.our products') }}</a></li>

              {{-- Login --}}
                @guest
                    <li>
                        <a href="{{ route('login') }}" class="text-xl font-medium text-green-600">Login</a>
                    </li>
                @endguest
                ...
                @auth
                    ...
                    @if(auth()->user()->is_admin)
                        <li>
                            <a href="{{ route('dashboard') }}" class="text-xl font-medium text-green-600">Dashboard</a>
                        </li>
                    @endif
                   ...
                   {{-- Logout --}}
                    <li>
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="text-xl font-medium text-green-600">Logout</button>
                        </form>
                    </li>
                @endauth
        ...
        <ul id="mobile-menu" class="md:hidden px-6 pt-4 pb-4 space-y-2 hidden bg-white shadow-lg">
            ...
            <li><a href="{{ route('categories.index') }}" class="block text-[#00d084] text-base font-medium hover:text-emerald-700">{{ __('messages.our products') }}</a></li>

           {{-- Login --}}
            @guest
                <li><a href="{{ route('login') }}" class="block text-[#00d084] text-base font-medium hover:text-emerald-700">Login</a></li>
            @endguest

            @auth
                @if(auth()->user()->is_admin)
                    <li><a href="{{ route('dashboard') }}" class="block text-[#00d084] text-base font-medium hover:text-emerald-700">Dashboard</a></li>
                @endif
                <li>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="block w-full text-left text-red-600 text-base font-medium hover:text-red-800">
                            Logout
                        </button>
                    </form>
                </li>
