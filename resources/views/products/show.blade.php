...
<section class="min-h-screen bg-gray-50 p-10">
    ...
    <div class="flex flex-col md:flex-row md:items-start gap-8 bg-white shadow-lg rounded-lg p-6 mt-20 ml-10 max-w-5xl">
        <div class="w-full">
            <h2 class="text-3xl font-bold mb-4 text-green-600">{{ $product->name }}</h2>
            <div class="flex flex-col md:flex-row gap-8">
    @if($product->imagepath)
        <img src="{{ asset($product->imagepath) }}" alt="{{ $product->name }}" class="w-96 h-96 object-cover rounded shadow">
    @endif
